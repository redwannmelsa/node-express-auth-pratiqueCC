...
router.get('/', function (req, res, next) {
  ...
  next();
}, fetchExser, function (req, res, next) {
  res.locals.filter = null;
  res.render('index', { user: req.user });
});

router.get('/active', ensureLoggedIn, fetchExser, function (req, res, next) {
  res.locals.exer = res.locals.exer.filter(function (todo) {
...
router.get('/completed', ensureLoggedIn, fetchExser, function (req, res, next) {
  res.locals.exer = res.locals.exer.filter(function (todo) {
    return todo.completed
  })
...
function fetchExser(req, res, next) {
  db.all('SELECT * FROM exer WHERE owner_id = ?', [
    ...
    res.locals.activeCount = exer.filter(function (todo) {
      return !todo.completed;
    }).length;
